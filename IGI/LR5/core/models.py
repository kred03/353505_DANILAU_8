...
# --- Новые модели для ЛР 1 HTML ---

class Product(models.Model):
    """Товары/услуги для каталога"""
...
class CompanyInfo(models.Model):
    ...
    certificate_image = models.ImageField('Изображение сертификата', upload_to='company/certificates/', blank=True, null=True)
    is_active = models.BooleanField('Активен', default=True)
    created_at = models.DateTimeField('Создано', auto_now_add=True)
    updated_at = models.DateTimeField('Обновлено', auto_now=True)
...
class CompanyHistory(models.Model):
    ...
    company_info = models.ForeignKey(CompanyInfo, on_delete=models.CASCADE, related_name='history', verbose_name='Информация о компании')
    created_at = models.DateTimeField('Создано', auto_now_add=True)
