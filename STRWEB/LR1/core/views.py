...
# Вспомогательный "каталог" для минимального сайта
MIN_CATALOG = {
    'lion': {'name': 'Лев', 'price': 100},
    'tiger': {'name': 'Тигр', 'price': 120},
    'giraffe': {'name': 'Жираф', 'price': 90},
}

# Минимальные вьюхи новостей (без оформления)
class NewsListViewMinimal(ListView):
    model = News
    template_name = 'news_db_list.html'
    context_object_name = 'news_list'
    paginate_by = 10

class NewsDetailViewMinimal(DetailView):
    model = News
    template_name = 'news_db_detail.html'
    context_object_name = 'news'

# Минимальный список FAQ с раскрывающимися ответами
class FAQListViewMinimal(ListView):
    model = FAQ
    template_name = 'faq_db_list.html'
    context_object_name = 'faqs'
    paginate_by = 20

# Корзина на сессиях (минимальная логика)
from django.urls import reverse

def cart_view(request):
    cart = request.session.get('cart', {})  # {item_key: qty}
    ...
    if action:
        ...
        return redirect('cart-min')

    # Готовим данные для отображения
    cart_items = []
    total = 0
    for key, qty in cart.items():
        info = MIN_CATALOG.get(key)
        if not info:
            continue
        price = info['price']
